...
function home(posts: Post[]) {
  ...
  return shell("juren233.top", `<main class="paged-home"><section class="home-screen brand-screen" aria-labelledby="brand-title"><div class="home-inner"><div class="mark">juren233.top</div><h1 id="brand-title" class="brand-title">juren233.top</h1><p class="brand-copy">一个持续更新的个人主站，用来放置当前项目、公开入口和最新一条值得被看到的站点更新。</p></div></section><section class="home-screen entry-screen"><div class="home-inner"><div class="entry-line"><div><div class="eyebrow">entry</div><h2 class="entry-title">留下一条消息</h2></div><div class="smallcaps">page 02</div></div><p class="entry-copy">联络入口单独占一整屏。它不做商业化包装，只负责把提案、协作想法和项目链接安静地接进来。</p><div class="entry-actions"><button class="btn btn-primary" type="button" data-open-modal="cooperation-modal">打开入口</button><span>如果你已经准备好内容，可以直接写清用途、联系方式和时间要求。</span></div></div></section><section class="home-screen update-screen"><div class="home-inner"><div class="update-line"><div><div class="eyebrow">current note</div><h2 class="update-title">最新动态</h2></div><div class="smallcaps">当前更新</div></div><div class="update-focus">${leadHtml}</div></div></section><section class="home-screen closing-screen" aria-label="closing brand page"><div class="home-inner"><h2 class="closing-brand">juren233.top</h2></div></section></main><div class="modal" id="cooperation-modal" aria-hidden="true"><div class="modal-backdrop" data-close-modal></div><section class="modal-panel" role="dialog" aria-modal="true" aria-labelledby="cooperation-title"><div class="modal-head"><div><div class="eyebrow">message</div><h2 id="cooperation-title">打开入口</h2><p>把用途、背景、项目链接和联系方式留在这里，我会在后台查看。</p></div><button class="icon-btn" type="button" aria-label="关闭联络入口" data-close-modal>×</button></div><form id="cooperation-form"><label>申请人名称<input name="name" placeholder="怎么称呼你" required/></label><label>联系方式<input name="contact" placeholder="邮箱 / QQ / Telegram / 微信" required/></label><label>项目说明<textarea name="projectSummary" placeholder="介绍你的内容、用途和展示方式" required></textarea></label><div class="split"><label>预期子域名 / 用途说明<input name="requestedSubdomain" placeholder="例如 demo、lab、share-guest"/></label><label>备注<input name="notes" placeholder="可填写时效、风格或其他说明"/></label></div><button class="btn btn-primary" type="submit">提交内容</button><div id="form-message" class="mark" style="letter-spacing:.08em;text-transform:none;">内容会直接进入后台列表。</div></form></section></div><script>const homeScroller=document.querySelector(".paged-home");const modal=document.getElementById("cooperation-modal");const openers=document.querySelectorAll('[data-open-modal="cooperation-modal"]');const closers=document.querySelectorAll("[data-close-modal]");const form=document.getElementById("cooperation-form");const message=document.getElementById("form-message");const screens=homeScroller instanceof HTMLElement?Array.from(homeScroller.querySelectorAll(".home-screen")):[];let isAnimating=false;const ease=(t)=>t<.5?4*t*t*t:1-Math.pow(-2*t+2,3)/2;const animateTo=(top)=>{if(!(homeScroller instanceof HTMLElement))return;const start=homeScroller.scrollTop;const distance=top-start;if(Math.abs(distance)<2)return;const duration=840;const started=performance.now();isAnimating=true;const tick=(now)=>{const progress=Math.min((now-started)/duration,1);homeScroller.scrollTop=start+distance*ease(progress);if(progress<1){requestAnimationFrame(tick);return;}homeScroller.scrollTop=top;isAnimating=false;};requestAnimationFrame(tick);};const moveScreen=(direction)=>{if(!(homeScroller instanceof HTMLElement)||!screens.length||isAnimating)return;const current=homeScroller.scrollTop+homeScroller.clientHeight*.2;let index=screens.findIndex((screen)=>screen.offsetTop<=current&&screen.offsetTop+screen.offsetHeight>current);if(index<0)index=0;const nextIndex=Math.max(0,Math.min(screens.length-1,index+direction));if(nextIndex===index)return;animateTo(screens[nextIndex].offsetTop);};const setModal=(open)=>{if(!modal)return;modal.classList.toggle("is-open",open);modal.setAttribute("aria-hidden",String(!open));if(homeScroller instanceof HTMLElement)homeScroller.style.overflowY=open?"hidden":"auto";};if(homeScroller instanceof HTMLElement&&screens.length>1&&!window.matchMedia("(prefers-reduced-motion: reduce)").matches){homeScroller.addEventListener("wheel",(event)=>{if(modal?.classList.contains("is-open")||isAnimating)return;if(Math.abs(event.deltaY)<=Math.abs(event.deltaX)||Math.abs(event.deltaY)<12)return;event.preventDefault();moveScreen(event.deltaY>0?1:-1);},{passive:false});}openers.forEach((node)=>node.addEventListener("click",()=>setModal(true)));closers.forEach((node)=>node.addEventListener("click",()=>setModal(false)));window.addEventListener("keydown",(event)=>{if(event.key==="Escape")setModal(false);});form?.addEventListener("submit",async(event)=>{event.preventDefault();if(!message)return;const data=new FormData(form);const payload=Object.fromEntries(data.entries());message.textContent='正在提交内容...';try{const response=await fetch('/api/applications',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});const json=await response.json();if(!response.ok)throw new Error(json.error||'提交失败');form.reset();message.textContent='内容已送达，我会在后台查看并跟进。';}catch(error){message.textContent=error instanceof Error?error.message:'提交失败，请稍后重试。';}});</script>`);
}
